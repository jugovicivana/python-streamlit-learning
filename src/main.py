...
with st.sidebar.form('book_form', clear_on_submit=True):
    new_name = st.text_input('Book Name', value='')
    new_author = st.text_input('Author', value='')
    new_user_rating = st.slider('User Rating', 0.0, 5.0, 0.0, 0.1)
    new_reviews = st.number_input('Reviews', min_value=0, step=1, value=0)
    new_price = st.number_input('Price', min_value=0, step=1, value=0)
    new_year = st.number_input('Year', min_value=2009, max_value=2022, step=1, value=2022)
...
fig = px.bar(top_authors, x='Count', y='Author', orientation='h',
             ...
             labels={'Author': 'Author', 'Count': 'Counts of Books Published'},
